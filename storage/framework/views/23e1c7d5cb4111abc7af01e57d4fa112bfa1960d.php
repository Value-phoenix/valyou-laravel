...
<div class="w-full overflow-auto background-shadow  mb-3 header-menu" style="width: 98.8%; margin-left: .7%">
	<div class="nav nav-tabs tab-select-nav px-2 m-2" id="nav-tab" role="tablist">
		<a class="active" id="nav-watch-tab" data-toggle="tab" href="#nav-watch" role="tab" aria-controls="nav-watch"
			aria-selected="true">
			PAY PER VIEW
			<span></span>
		</a>
		<a class id="nav-earn-tab" data-toggle="tab" href="#nav-earn" role="tab" aria-controls="nav-profile"
			aria-selected="false">
			WATCH & EARN
		</a>
		<a class id="nav-valyou-tab" data-toggle="tab" href="#nav-valyou" role="tab" aria-controls="nav-contact"
			aria-selected="false">
			VALYOU PLAYLIST
		</a>
		<a class="" id="nav-watch-tab" data-toggle="tab" href="#nav-watch" role="tab" aria-controls="nav-watch"
			aria-selected="true">
			PROMOTED VIDEOS
			<span></span>
		</a>
...
<div class="tab-content " id="nav-watch">
	<div id="mobile-div1">
		<div class="mo-video-part row ml-0 mr-0">
			...
			<div class="col-sm-12 col-md-12 col-lg-4  " style="max-height: 70vh">
				...
				<div id="video-list-des">
					<div class="all-video-list card  p-2 desktop-mode">
						<div class="p-3">
							<h2 class="txt-main all-videos-text">All Videos</h2>
						</div>
						<!-- <hr class="m-0 mb-2"> -->
						<div class="special-video-part ">
							...
							<div class="especailly-part inline-flex w-full background-shadow px-2 my-2 pt-2 pb-2"
								onclick="selVideoDetail(<?php echo e($row->id); ?>, 1)" style="width: 98.8%; margin:.7%">
								...
								<div class="video-detail-info py-1 pl-2">
									...
									<p style="font-size: small; font-weight: 200;">Song Valyou: <strong
											id="valyou-val">$<?php echo e(numberformat(@$row->artist->stock_value)); ?></strong></p>
									<h6 class=<?php echo e($row->artist->change_stock<0?'txt-main': 'text-green'); ?>><small>Stock
												Price
												&nbsp;<span><img class="" width="15" height="15"
														src="<?php echo e($row->artist->change_stock<0?'https://testvps.nwlogics.com/public/assets/images/valyoux/pink_arrow_circle_down.svg': 'https://testvps.nwlogics.com/public/assets/images/valyoux/green_arrow_price_going_up.svg'); ?>"></span>&nbsp;&nbsp;<strong>$<?php echo e($row->artist->stock_value); ?>VXD</strong>&nbsp;<strong><?php echo e($row->artist->change_stock
			...
			<div class="mo-userinfo px-1">
				...
				<div class="mo-userinfo-content">
					...
					<div class="personal-detail-info-category">
						<p><strong class="song-title">Song Title:</strong> <strong class="song-from"> &nbsp;Started From
								The Bottom</strong></p>
						&nbsp;&nbsp;&nbsp;
						<p><strong class="song-valyou">Song Valyou:</strong><strong class="song-valyou-value">&nbsp;
								$1,000,000,24,567</strong></p>
						&nbsp;&nbsp;&nbsp;
						<p><strong class="song-category">Music Artist Brand Listing Category:</strong> <strong
								class="song-type">&nbsp; Major Artist</strong></p>
					</div>
				</div>
			</div>
		</div>
		<div class="mo-content  px-2 background-shadow" style="margin-left: .7%">
			...
			<div>
				...
				<div class="all-video-list card  p-2 mobile-mode">
					<div class="p-3">
						<h2 class="txt-main">All Videos</h2>
					</div>
					<hr class="m-0 mb-2">
					<div class="special-video-part ">
						...
						<div class="especailly-part inline-flex w-full background-shadow px-2 my-2 "
							...
							<div class="video-detail-info p-3">
								...
								<h6 class=<?php echo e($row->artist->change_stock<0?'txt-main': 'text-green'); ?>><small>Stock Price
											&nbsp;<span><img class="" width="15" height="15"
													src="<?php echo e($row->artist->change_stock<0?'https://testvps.nwlogics.com/public/assets/images/valyoux/pink_arrow_circle_down.svg': 'https://testvps.nwlogics.com/public/assets/images/valyoux/green_arrow_price_going_up.svg'); ?>"></span>&nbsp;&nbsp;<strong>$<?php echo e($row->artist->stock_value); ?>VXD</strong>&nbsp;<strong><?php echo e($row->artist->change_stock
												<0?'': '+'); ?><?php echo e($row->artist->change_stock); ?>%
											</strong></small></h6>
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="staticBackdropLabel"><b class="color-pinkk">Pay $0.03 cents and
								Watch</b>
						</h5>
					...
					<div class="modal-body">
						...
						<p>If you do not wish to be charged again each time you listen to this song, you have the option
							to click and Valyou this song
							for an amount of your choice in order to receive unlimited plays of this song as well Vip
							artist-fan reward points the more you Valyou .</p>
...
<script>
	// desktop
	// comment btn
	$(document).on('click', '.dt-comment-btn', function () {
		$('#video-list-des').hide();
		$('.header-menu').hide();
	...
	//
	//promote
	$(document).on('click', '.dt-promote-btn', function () {
		$('#video-list-des').hide();
		$('.header-menu').hide();
	...
	//valyou song btn
	//promote
	$(document).on('click', '.dt-valyou-btn', function () {
		$('#video-list-des').hide();
		$('.header-menu').hide();
	...
	//close btun
	$(document).on('click', '.mo-promote-btn', function () {
		$('#video-list-des').show();
		$('.header-menu').show();
	})
	...
	// mobile
	// comment btn
	$(document).on('click', '.mo-comment-btn', function () {
		$('#video-list-des').hide();
		$('.header-menu').hide();
	...
	//close btun
	$(document).on('click', '.dt-close_btn', function () {
		$('#video-list-des').show();
		$('.header-menu').show();
	})
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Administrator\Documents\Valyou X\valyoux_backend(12.27)\valyoux_backend\resources\views/watch2.blade.php ENDPATH**/ ?>